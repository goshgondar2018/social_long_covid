...
DAG1_combined_cohort_plot <- DAG1_combined_cohort %>%
  tidy_dagitty() %>%
...
DAG1_combined_cohort_plot

ggsave("./figures/DAG_1_combined_cohort.png")

DAG2_combined_cohort <- dagitty('dag {
  ...
  "SES indicator" [pos="-2.240,-0.428"]
  Country [pos="-2.240,0.615"]
...
DAG2_combined_cohort_plot <- DAG2_combined_cohort %>%
  tidy_dagitty() %>%
  ggplot(aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_dag_edges(edge_colour='grey') +
  theme_dag()+
  geom_dag_node(col='orange') +
  geom_dag_text(col='black',size=3)
DAG2_combined_cohort_plot

ggsave("./figures/DAG_2_combined_cohort.png")
...
DAG1_Norway <- dagitty('dag {
  ...
  "Long COVID QALDs" [outcome,pos="2,-0.4"]
  ...
  Smoking -> "Long COVID QALDs"
  "COVID Vaccination Status" -> "Long COVID QALDs"
}')
...
## combined cohort
DAG1_combined_cohort <- dagitty('dag {
  "Chronic Cardiac Disease (not htn)" [pos="-0.2,-0.704"]
  "Chronic Pulmonary Disease (not asthma)" [pos="-0.2,-0.005"]
  "Diabetes (type not specified)" [pos="-0.2,-1.385"]
  "Type 1 Diabetes" [pos="-0.2,0.357"]
